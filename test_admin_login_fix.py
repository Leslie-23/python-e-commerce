...
def test_admin_login_response():
    ...
    try:
        response = requests.post("http://127.0.0.1:5000/auth/login", json=admin_login_data)
        ...
        if response.status_code == 200:
            ...
            if 'isVerified' in user_data:
                print(f"   ✅ isVerified (camelCase): {user_data['isVerified']}")
            else:
                print("   ❌ isVerified (camelCase): NOT FOUND")
                
            if 'is_verified' in user_data:
                print(f"   ⚠️  is_verified (snake_case): {user_data['is_verified']} (should be removed)")
            else:
            ...
            if 'is_admin' in user_data:
                print(f"   ⚠️  is_admin (snake_case): {user_data['is_admin']} (should be removed)")
            else:
            ...
            if user_data.get('isVerified') is True and 'is_verified' not in user_data:
                ...
                print("   - Frontend should no longer redirect admin to OTP verification")
                return True
            else:
